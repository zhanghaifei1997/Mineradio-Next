...
- 地形模式优化：锁定相机参数，歌单弹窗不再推移场景。
- 地形网格动态缩放：网格尺寸跟随相机距离自动缩放。
- 自动布局适配：地形模式下歌单架与歌词自动调整位置。
- 地形专属相机：独立相机角度与视觉质量调优。
- 互斥逻辑加强：地形可视化与预设动效在动画循环每帧互斥生效。
- 地形视觉修复：修复比例、层级、位置等渲染 bug。

## v1.1.4

- 重新生成 README.md，按项目介绍、功能特色、下载、开发运行、技术架构重新组织内容。

## v1.1.3

- 登录安全加固：网易云 WebView 登录窗口重建时自动清除旧 cookie，防止残留 MUSIC_U cookie 被误判为有效登录；捕获 cookie 后增加服务端校验（login_status API），profile 为 null 时继续轮询而非直接放行。
- 退出登录清理：网易云与 QQ 退出登录时调用 clear_all_browsing_data() 并 destroy() 替代仅 close()，彻底释放 WebView2 资源并防止下次登录读到旧会话。
- 登录状态判定修复：enrich_login_body 将 profile: null（API 返回 code:200 但会话已失效）视同 profile 缺失，避免误报"已登录"；/api/login/cookie 验证失败时返回明确提示信息"网易云会话已失效，请重新登录"。
- QQ 音乐登录路由：新增 /api/qq/login/cookie、/api/qq/login/status、/api/qq/logout、/api/qq/user/playlists 四个 HTTP 路由，与网易云登录体系对齐。
- 播客订阅接口：新增 /api/podcast/my/items 路由，支持按 key（collect）分页获取用户订阅的播客列表。
- 前端 UI：index.html 多处视觉与交互优化（歌单面板、控制台、粒子预设、双账号模式等）。
- 路由注册整理：server/routes/mod.rs 路由注册重构，QQ 路由与播客路由统一拆分至独立模块。

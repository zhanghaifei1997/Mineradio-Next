...
function handleGlobalMouseMove(e: MouseEvent) {
  ...
  revealCursorForActivity()
}

// ---- Cursor auto-hide (2.5s idle) ----
...
function handleCursorActivityEvent() {
  revealCursorForActivity()
}

const cursorActivityEvents = ['pointermove', 'mousedown', 'wheel', 'touchstart'] as const

function setupCursorAutoHide() {
  cursorActivityEvents.forEach(type =>
    window.addEventListener(type, handleCursorActivityEvent, { passive: true, capture: true })
  )
  revealCursorForActivity()
}

function cleanupCursorAutoHide() {
  clearCursorAutoHideTimer()
  setCursorHidden(false)
  cursorActivityEvents.forEach(type =>
    window.removeEventListener(type, handleCursorActivityEvent)
  )
}
...
<template>
  <div class="main-container" :class="{ 'fx-eco': fx.settings.performanceQuality === 'eco', 'mini-mode': isMiniMode, 'drag-over': isDragging }">
    ...
    <template v-if="!isMiniMode">
      ...
      <PlayerBar
        @show-queue="toggleQueuePanel"
        @show-local="toggleLocalPanel"
        @open-song-detail="openSongDetail"
        @open-artist-detail="(id: string, src: string) => openArtistDetail({ id, name: '' } as any, src)"
      />
...
.side-panel {
  ...
  pointer-events: auto;
  border-radius: var(--saved-panel-glass-radius, 50px);
  overflow: hidden;
...
.slide-right-enter-active {
  transition: transform 0.55s cubic-bezier(.16,1,.3,1), opacity 0.45s cubic-bezier(.16,1,.3,1), filter 0.55s cubic-bezier(.16,1,.3,1);
}
.slide-right-leave-active {
  transition: transform 0.26s cubic-bezier(.5,0,.7,1), opacity 0.2s cubic-bezier(.5,0,.7,1), filter 0.26s cubic-bezier(.5,0,.7,1);
}
...
/* 桌面模式：工具栏下移至标题栏下方 (44px titlebar + 14px gap = 58px) */
:global(body.desktop-titlebar-active) .top-toolbar {
  top: 58px;
  padding: 0 24px;
}

:global(body.desktop-titlebar-active) .side-panels {
  top: 44px;
}
</style>

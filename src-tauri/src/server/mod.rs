...
pub async fn start_server(state: Arc<AppState>) -> u16 {
    ...
    let server = HttpServer::new(move || {
        ...
        App::new()
            ...
            .route("/api/qq/logout", web::get().to(routes::stub_qq_logout))
            .route("/api/qq/user/playlists", web::get().to(routes::stub_qq_user_playlists))
            .route("/api/qq/login/cookie", web::post().to(routes::stub_qq_login_cookie))
            .route("/api/qq/login/status", web::get().to(routes::stub_qq_login_status))
            .route("/api/qq/search", web::get().to(routes::stub_qq_search))
            .route("/api/qq/song/url", web::get().to(routes::stub_qq_song_url))
            .route("/api/qq/lyric", web::get().to(routes::stub_empty_object))
            .route("/api/qq/artist/detail", web::get().to(routes::stub_empty_object))
            .route("/api/qq/playlist/tracks", web::get().to(routes::stub_qq_playlist_tracks))
            .route("/api/qq/song/comments", web::get().to(routes::stub_qq_song_comments))
            .route("/api/beatmap/cache/status", web::get().to(routes::stub_beatmap_cache_status))
            .route("/api/beatmap/cache", web::get().to(routes::stub_beatmap_cache_get))
            .route("/api/beatmap/cache", web::post().to(routes::stub_beatmap_cache_post))
            .route("/api/podcast/my/items", web::get().to(routes::stub_podcast_my_items))
            .route("/api/podcast/dj-beatmap", web::get().to(routes::stub_dj_beatmap))
            .route("/api/update/latest", web::get().to(routes::stub_update_latest))
            .route("/api/update/download", web::post().to(routes::stub_update_not_available))
            .route("/api/update/download/status", web::get().to(routes::stub_update_not_available))
            .route("/api/update/patch", web::post().to(routes::stub_update_not_available))
            .route("/api/update/patch/status", web::get().to(routes::stub_update_not_available))
            .route("/api/weather/radio", web::get().to(routes::stub_weather_radio))
            .route("/api/weather/ip-location", web::get().to(routes::stub_weather_ip_location))
